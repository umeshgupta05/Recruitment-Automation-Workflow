...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (candidatesRes.status === 'fulfilled') setCandidates(candidatesRes.value.data.candidates || []);
  ...
  const pipelineStages = stats
    ? [
        { key: 'applied', label: 'Applied', count: stats.applied, color: 'bg-neutral-400' },
        { key: 'scored', label: 'Scored', count: stats.scored, color: 'bg-primary-500' },
        { key: 'shortlisted', label: 'Shortlisted', count: stats.shortlisted, color: 'bg-blue-500' },
        { key: 'interview_scheduled', label: 'Interview', count: stats.interviews, color: 'bg-warning-500' },
        { key: 'offer_extended', label: 'Offer', count: stats.offers, color: 'bg-success-500' },
        { key: 'rejected', label: 'Rejected', count: stats.rejected, color: 'bg-danger-500' },
      ]
    : [];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-neutral-100">Recruitment hub</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 card">
          <h3 className="text-sm font-semibold text-neutral-300 mb-4">Recent candidates</h3>
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Workflow runs */}
          <div className="card">
            <h3 className="text-sm font-semibold text-neutral-300 mb-4">Workflow runs</h3>
            <div className="space-y-3">
              {workflows.length > 0 ? (
                workflows.slice(0, 4).map((run, i) => (
                  <div key={run.id || i} className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <span className="text-xs text-neutral-500 w-16 text-right">{timeAgo(run.started_at)}</span>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-sm text-neutral-500 text-center py-4">No workflow runs yet</p>
              )}
            </div>
          </div>
          ...
          {/* Upload zone */}
          <div className="card">
            <h3 className="text-sm font-semibold text-neutral-300 mb-4">Upload resume</h3>
      ...
      <div className="card">
        <h3 className="text-sm font-semibold text-neutral-300 mb-4">Weekly application volume</h3>
        <div className="h-64">
          {stats?.weeklyVolume && stats.weeklyVolume.some((w) => w.count > 0) ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-xs text-neutral-500 mt-1">Upload resumes to see weekly trends</p>
              </div>
            </div>
          )}
        </div>
